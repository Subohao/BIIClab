...
for i in range(row_A):# i == frame_num
    ...
    ##################################################################################### 
        ...
        frame_accum += 1 
        if frame_accum == 300:           
            recog_A = AnswerA[i-300:i]
            recog_B = AnswerB[i-300:i]
            feature_index1 = np.where(feature[:,0] <= i-1) 
            feature_index2 = np.where(feature[feature_index1,0] >= i-300)
            ...
            if (SVM_validation(Answer_A_feat_train, Answer_B_feat_train, recog_A_feat, recog_B_feat, 10, 244, 10) == True ):
                AnswerA[i-300:,:],AnswerB[i-300,:]=AnswerB[i-300:,:].copy(),AnswerA[i-300,:].copy()
